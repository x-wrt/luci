...
return baseclass.extend({
	...
	render(tree) {
		...
		const toggleBtn = document.querySelector('#menubar > .navigation') || document.querySelector('#menubar > h2');
		if (toggleBtn) {
			toggleBtn.addEventListener('click', ui.createHandlerFn(this, 'handleSidebarToggle'));
		}

		// 点击空白处收起菜单
		document.addEventListener('click', (ev) => {
			const bar = document.querySelector('#mainmenu');
			const btn = document.querySelector('#menubar > .navigation') || document.querySelector('#menubar > h2');

			if (bar && btn && bar.classList.contains('active')) {
				if (!bar.contains(ev.target) && !btn.contains(ev.target)) {
	...
	handleMenuExpand(ev) {
		...
		// 💡 因为页面加载时已经同步，现在只需要简单地检查 active 即可！
		const isExpanded = li.classList.contains('active');
	...
	renderMainMenu(tree, url, level) {
		...
		children.forEach(child => {
			...
			const activeClass = 'mainmenu-item-%s%s'.format(child.name, isActive ? ' selected active' : '');
